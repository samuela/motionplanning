...
################################################################################
def _dpmp_surgery():
    from DPMPPlanner import DPMPPlanner
    og.DPMPPlanner = DPMPPlanner
    og.planners.plannerMap['ompl.geometric.DPMPPlanner'] = DPMPPlanner.PARAM_DEFS
_dpmp_surgery()
################################################################################
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def createPlanner(self):
        ...
        # planner = DPMPPlanner(si)
        # print self.mainWidget.plannerWidget.geometricPlanning.plannerList
    ...
    def solve(self):
        ...
        if solved:
            if self.isGeometric:
                ...
                print 'planner name', self.omplSetup.getPlanner().getName()
...
class GLViewer(QtOpenGL.QGLWidget):
    ...
    def paintGL(self):
        ...
        if self.drawPlannerData and self.plannerDataList:
            ...
            if self.drawPlannerData == 0:
                # don't plot states or path
                pass
            elif self.drawPlannerData == 1:
                # plot states only
                GL.glCallList(self.plannerDataList)
            elif self.drawPlannerData == 2:
                # plot states and path
                GL.glCallList(self.plannerDataList)
                GL.glCallList(self.plannerDataList + 1)
...
class GeometricPlannerWidget(PlannerHelperWidget):
    def __init__(self):
        ...
        self.plannerSelect.setCurrentIndex([p[0] for p in self.plannerList].index('ompl.geometric.DPMPPlanner'))
